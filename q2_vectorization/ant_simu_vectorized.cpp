...
void advance_time_vectorized( const fractal_land& land, pheronome& phen, 
                               ...
                               ant_vectorized& ants, std::size_t& cpteur,
...
int main(int nargs, char* argv[])
{
    ...
    ant_vectorized::set_exploration_coef(eps);
    
    // On va créer des fourmis un peu partout sur la carte :
    ant_vectorized ants(nb_ants);
    ants.initialize(land, seed);
    
    // on maintient une copie nécessaire pour l'affichage
    std::vector<ant> render_ants;
    render_ants.reserve(nb_ants);
    // we ignore the pseudo‑random seed for rendering
    for(std::size_t i=0;i<nb_ants;++i)
        render_ants.emplace_back(position_t{0,0},0);

    // On crée la carte des phéromones
    pheronome phen(land.dimensions(), pos_food, pos_nest, alpha, beta);

    Window win("Ant Simulation (Vectorized)", 2*land.dimensions()+10, land.dimensions()+266);
    Renderer renderer( land, phen, pos_nest, pos_food, render_ants );
    ...
    while (cont_loop) {
        ...
        // update rendering copy to current positions/states
        const auto &pos = ants.get_positions();
        const auto &sts = ants.get_states();
        // rebuild render vector each iteration to avoid assignment operator
        render_ants.clear();
        for(std::size_t i=0;i<ants.size();++i) {
            render_ants.emplace_back(pos[i],0);
            if (sts[i] == ant_vectorized::loaded)
                render_ants.back().set_loaded();
        }
    ...
    std::cout << "\n========== TIME MEASUREMENTS (Vectorized Version) ==========" << std::endl;
