...
public:
    ...
    // dimension getter for MPI partitioning
    size_t dim() const { return m_dim; }

    /**
     * @brief synchronise la carte des phéronomes avec tous les processus
     * @details effectue un MPI_Allreduce avec l'opération MAX sur chaque
     *          composante afin que tous les processus disposent de la valeur
     *          maximale observée pour chaque cellule. Nécessite MPI_Init() au
     *          préalable.
     */
    void mpi_sync(MPI_Comm comm) {
        int count = static_cast<int>(m_map_of_pheronome.size() * 2);
        std::vector<double> sendbuf(count);
        for (size_t i = 0; i < m_map_of_pheronome.size(); ++i) {
            sendbuf[2*i]   = m_map_of_pheronome[i][0];
            sendbuf[2*i+1] = m_map_of_pheronome[i][1];
        }
        std::vector<double> recvbuf(count);
        MPI_Allreduce(sendbuf.data(), recvbuf.data(), count, MPI_DOUBLE, MPI_MAX, comm);
        for (size_t i = 0; i < m_map_of_pheronome.size(); ++i) {
            m_map_of_pheronome[i][0] = recvbuf[2*i];
            m_map_of_pheronome[i][1] = recvbuf[2*i+1];
        }
    }

    /**
     * @brief copier la i-ème ligne (indice interne) dans un buffer (2 composantes par cellule)
     */
    void pack_row(size_t i, double* buf) const {
        size_t stride = m_stride;
        for(size_t j = 0; j < stride; ++j) {
            buf[2*j]   = m_map_of_pheronome[i*stride + j][0];
            buf[2*j+1] = m_map_of_pheronome[i*stride + j][1];
        }
    }
    /**
     * @brief décompresser un buffer dans la i-ème ligne
     */
    void unpack_row(size_t i, const double* buf) {
        size_t stride = m_stride;
        for(size_t j = 0; j < stride; ++j) {
            m_map_of_pheronome[i*stride + j][0] = buf[2*j];
            m_map_of_pheronome[i*stride + j][1] = buf[2*j+1];
        }
    }
